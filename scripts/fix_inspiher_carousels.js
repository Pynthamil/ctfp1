  ...
  const itemsHtml = items.map(group => {
    if (group.length === 1) {
      ...
      const innerHtml = group.map((f, i) => {
        // Add a very subtle right border to distinguish slides in the carousel if needed, 
        // but typically carousels just touch. Let's just let them touch.
...
console.log('Fixed Inspiher Carousels');
